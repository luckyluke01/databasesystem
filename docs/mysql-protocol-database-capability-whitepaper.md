...
| 事务 | 不支持 | - | 本系统定位为查询服务，不提供事务语义 | - | 事务语法拒绝测试 |  |
...
本系统定位为统一查询服务，不支持事务能力。对外白皮书不应承诺 `BEGIN`、`COMMIT`、`ROLLBACK`、隔离级别、行锁、表锁、`SELECT FOR UPDATE` 等事务或锁语义。
...
- 明确声明本系统不支持事务、隔离级别和锁语义
- 业务是否依赖事务提交、回滚、隔离级别或行锁能力；如依赖，应判定为不适用或要求业务改造
- 是否保证读己之写
- 是否可能读到延迟数据
- 故障切换期间语义
...
| 是否需要写入？写入 QPS 多少？ |  |
| 是否依赖事务、回滚、隔离级别或锁能力？如依赖，本系统不适用或需业务改造。 |  |
| 单表预计数据量？ |  |
| 单次查询预计扫描多少数据？ |  |
| 单次查询预计返回多少数据？ |  |
| 并发连接数多少？ |  |
